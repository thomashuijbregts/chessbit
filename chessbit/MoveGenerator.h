...
namespace movegen {
    ...
    *****************************************************/
    ...
    struct Batch {
        ...
        template <int depth, bool k, class Build>
        __forceinline void add(Build&& build) noexcept
        {
            BoardState* b;

            if constexpr (k) b = &king[kSize++];
            else             b = &normal[nSize++];
    ...
    struct NullEval {
        ...
        template <bool side>
        __forceinline void take(U64& nodes, U64& attacks, int from, U64 map) noexcept {
            ...
            U64 coef = 0;
            if (f & maps.pFwdFrom1) coef += 1;
            if (f & maps.capOne)    coef -= 1;
            if (f & maps.capTwo)    coef -= 2;
            ...
            else if ((f & maps.pFwdFromDbl) && (pawnsAtkForward<side>(f) & q)) nodes--;

            nodes += Bitcount(q & maps.capOne) + 2 * Bitcount(q & maps.capTwo) - Bitcount(q & maps.pFwdTo1) - 2 * Bitcount(q & maps.pFwdTo2);

            if (maps.promoOn) [[unlikely]]
                nodes += 3 * Bitcount(q & maps.capOneLR) + 6 * Bitcount(q & maps.capTwoLR) - 3 * Bitcount(q & maps.t1LR);
        }

        template <bool side>
        __forceinline void takeKing(U64& nodes, U64& attacks, U64 kM, U64 map) noexcept {
            if (kM & map) return;
            ...
            if (q & maps.cstlBit) nodes--;

            nodes += NULL_KING_COEFF[SquareOf(kM & maps.pFwdFrom1)] * cnt;

            if (kM & maps.pFwdFrom2) {
                nodes += cnt << 1;
                if (pawnsAtkForward<!side>(kM) & q) nodes--;
            }
            else if ((kM & maps.pFwdFromDbl) && (pawnsAtkForward<side>(kM) & q)) nodes--;

            nodes -= Bitcount(q & maps.pFwdTo1) + 2 * Bitcount(q & maps.pFwdTo2);
            if (maps.promoOn) [[unlikely]]
                nodes -= 3 * Bitcount(q & maps.t1LR);
        }

        template <bool side>
        __forceinline void takePawn(U64& nodes, U64& pawnsFwd, U64& pawnsDbl, U64 map) noexcept {
            ...
            const U64 ePL = left(quietD) & maps.ePCR;
            const U64 ePR = right(quietD) & maps.ePCL;
            if (ePL | ePR) nodes += Bitcount(ePL) + Bitcount(ePR);
            if (quietF & maps.cstlBit) nodes--;

            nodes -= Bitcount(pF & maps.capOne) + Bitcount(pD & maps.capOne);
            nodes += Bitcount(pF & maps.pFwdFrom1nDbl);
            nodes += Bitcount(quietB & maps.capOne);
            nodes -= Bitcount(quietB & maps.pFwdTo1);

            if (maps.capTwo) [[unlikely]] {
                nodes -= 2 * (Bitcount(pF & maps.capTwo) + Bitcount(pD & maps.capTwo));
                nodes += 2 * Bitcount(quietB & maps.capTwo);
            }
    ...
    ForceInline void enumMoves(U64& nodes, U64 moves, int from, const BoardState& board, U64 discovers, Batch* batch) noexcept {
        Bitloop(moves) {
            ...
            if constexpr (depth >= 3 && useTT) {
                if constexpr (piece == Piece::King) batch->add<depth, true>([&] { return board.make<piece, side, capture, kMoved, (useTT && depth != 2)>(from, to, board, discovers); });
                else                                batch->add<depth, false>([&] { return board.make<piece, side, capture, kMoved, (useTT && depth != 2)>(from, to, board, discovers); });
            }
            else {
    ...
    ForceInline void makeMove(U64& nodes, int from, int to, const BoardState& board, U64 discovers, Batch* batch) noexcept {
        if constexpr (depth >= 3 && useTT) {
            batch->add<depth, false>([&] { return board.make<piece, side, capture, kMoved, (useTT && depth != 2)>(from, to, board, discovers); });
            return;
        }
    ...
    ForceInline void makeEnPassant(U64& nodes, int from, const BoardState& board, U64 discovers, Batch* batch) noexcept {
        if constexpr (depth >= 3 && useTT) {
            batch->add<depth, false>([&] { return board.makeEnPassant<side, (useTT && depth != 2)>(from, board.eP, board); });
            return;
        }
    ...
    ForceInline void makeDoublePush(U64& nodes, int from, int to, const BoardState& board, U64 discovers, Batch* batch) noexcept {
        if constexpr (depth >= 3 && useTT) {
            batch->add<depth, false>([&] { return board.makeDoublePush<side, (useTT && depth != 2)>(from, to, board, discovers); });
            return;
        }
    ...
    ForceInline void makePromotionMoves(U64& nodes, int from, int to, const BoardState& board, U64 discovers, Batch* batch) noexcept {
        if constexpr (depth >= 3 && useTT) {
            batch->add<depth, false>([&] { return board.makePromotion<Piece::Knight, side, capture, kMoved, (useTT && depth != 2)>(from, to, board, discovers); });
            batch->add<depth, false>([&] { return board.makePromotion<Piece::Bishop, side, capture, kMoved, (useTT && depth != 2)>(from, to, board, discovers); });
            batch->add<depth, false>([&] { return board.makePromotion<Piece::Rook, side, capture, kMoved, (useTT && depth != 2)>(from, to, board, discovers); });
            batch->add<depth, false>([&] { return board.makePromotion<Piece::Queen, side, capture, kMoved, (useTT && depth != 2)>(from, to, board, discovers); });
            return;
        }
    ...
    ForceInline void makeCastling(U64& nodes, const BoardState& board, Batch* batch, NullEval& null, NullMaps* maps) noexcept {
        ...
        else if constexpr (depth >= 3 && useTT) batch->add<depth, true>([&] { return board.makeCastling<castlingSide, (useTT && depth != 2)>(board); });
        else {
            const BoardState newBoard = board.makeCastling<castlingSide, (useTT && depth != 2)>(board);
    ...
    ForceInline U64 allMoves(const BoardState& board, NullMaps* maps) noexcept {
        ...
        */
        ...
        if constexpr (depth == 2) null.takeKing<side>(nodes, attacks, board.kM, null.king);
        ...
        */
        ...
        if (allPins) {
            ...
            Bitloop(bitboard)
            {
                ...
                if ((1ULL << from) & board.qM) {
                    if constexpr (depth == 2) null.take<side>(nodes, attacks, from, null.queen);
                    ...
                    if constexpr (depth == 2) null.take<side>(nodes, attacks, from, null.bishop);
            ...
            Bitloop(bitboard)
            {
                ...
                if constexpr (depth == 2) null.take<side>(nodes, attacks, from, isQueen ? null.queen : null.rook);
        ...
        if constexpr (depth == 1) {
            ...
            if constexpr (depth == 2) null.takePawn<side>(nodes, pawnsFwd, pawnsDbl, null.pawn);
        ...
        */
        ...
        Bitloop(bitboard)
        {
            ...
            if constexpr (depth == 2) null.take<side>(nodes, attacks, from, null.knight);
        ...
        */
        ...
        Bitloop(bitboard)
        {
            ...
            if constexpr (depth == 2) null.take<side>(nodes, attacks, from, null.rook);
